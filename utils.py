...
class Transfer():
    ...
    def __init__(self, betas_fr, betas_it, basis_fr, basis_it, y_fr, y_it,rho = None, K=75):
        ...
        common_idx,fr_to_it, it_to_fr =  np.intersect1d(betas_it.index, betas_fr.index , return_indices=True)
    ...
    def gradient_descent(self,beta, y, basis, t):
        ...
        y_te = torch.tensor(y[:t], requires_grad=False) # No data leakage. We only use the data up to time t
    ...
    def gam_delta_ft(self,country_src='it',country_dst='fr'):
        ...
        for t in tqdm(range(l)):
            # Compute the prediction at time t
            beta_t = self.gam_delta_ft_t(t,country_src=country_src,country_dst=country_dst)
            tuning.append(beta_t@basis[t].T)
        return tuning
